...
SmartacPatternFormatter::SmartacPatternFormatter(void):	_localTime(false)
{
}


SmartacPatternFormatter::~SmartacPatternFormatter(void)
{
}

void SmartacPatternFormatter::format(const Poco::Message& msg, std::string& text) {
	...
	for (std::vector<PatternAction>::iterator ip = _patternActions.begin(); ip != _patternActions.end(); ++ip)
	{
		...
		case 'I': text.append(NumberFormatter::formatHex(msg.getTid(), 6, true)); break;
...
void SmartacPatternFormatter::parsePattern()
{
...
void SmartacPatternFormatter::setProperty(const std::string& name, const std::string& value)
{
...
std::string SmartacPatternFormatter::getProperty(const std::string& name) const
{
	if (name == PROP_PATTERN)
		return _pattern;
	else if (name == PROP_TIMES)
...
const std::string& SmartacPatternFormatter::getPriorityName(int prio)
{
